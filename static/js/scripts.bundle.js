...
const KTApp = (function () {
  ...
  return {
    ...
    block (target, options) {
      ...
      const spinner = '<span class="spinner ' + version + ' ' + state + ' ' + size + '"></span'
...
var KTUtil = (function () {
  ...
  return {
    ...
    outerHeight (el, withMargic = false) {
    ...
    btnWait (el, cls, message, disable = true) {
      if (!el) {
        return
      }
    ...
    isOffscreen (el, direction, offset = 0) {
...
'use strict';
(function ($) {
  const pluginName = 'KTDatatable'
  const pfx = ''
  const util = KTUtil
  const app = KTApp

  if (typeof util === 'undefined') { throw new TypeError('Util class is required and must be included before ' + pluginName) }
  ...
  $.fn[pluginName] = function (options) {
    ...
    const Plugin = {
      ...
      extractTable () {
        const columns = []
        const headers = $(datatable).find('tr:first-child th').get().map(function (cell, i) {
          ...
          for (const ii in options.columns) {
      ...
      lockTable () {
        const lock = {
          ...
          enable () {
            const enableLock = function (tablePart) {
              ...
              const lockLeft = $('<div/>').addClass(pfx + 'datatable-lock ' + pfx + 'datatable-lock-left')
              const lockScroll = $('<div/>').addClass(pfx + 'datatable-lock ' + pfx + 'datatable-lock-scroll')
              const lockRight = $('<div/>').addClass(pfx + 'datatable-lock ' + pfx + 'datatable-lock-right')

              $(tablePart).find('.' + pfx + 'datatable-row').each(function () {
                // create new row for lock columns and pass the data
                const rowLeft = $('<tr/>').addClass(pfx + 'datatable-row').data('obj', $(this).data('obj')).appendTo(lockLeft)
                const rowScroll = $('<tr/>').addClass(pfx + 'datatable-row').data('obj', $(this).data('obj')).appendTo(lockScroll)
                const rowRight = $('<tr/>').addClass(pfx + 'datatable-row').data('obj', $(this).data('obj')).appendTo(lockRight)
      ...
      setupHover () {
        ...
        $(datatable.tableBody).find('.' + pfx + 'datatable-cell').off('mouseenter', 'mouseleave').on('mouseenter', function () {
          ...
          // normal table
          const row = $(this).closest('.' + pfx + 'datatable-row').addClass(pfx + 'datatable-row-hover')
          ...
          // normal table
          const row = $(this).closest('.' + pfx + 'datatable-row').removeClass(pfx + 'datatable-row-hover')
          const index = $(row).index() + 1
      ...
      adjustLockContainer () {
        ...
        const containerWidth = $(datatable.tableHead).width()
        let lockLeft = $(datatable.tableHead).find('.' + pfx + 'datatable-lock-left').width()
        let lockRight = $(datatable.tableHead).find('.' + pfx + 'datatable-lock-right').width()
        ...
        const lockScroll = Math.floor(containerWidth - lockLeft - lockRight)
      ...
      dragResize () {
        ...
        $(datatable.tableHead).find('.' + pfx + 'datatable-cell').mousedown(function (e) {
          ...
          if (pressed) {
            ...
            const ifLocked = $(start).closest('.' + pfx + 'datatable-lock')

            if (ifLocked) {
              const lockedIndex = $(ifLocked).index()
      ...
      initHeight () {
        if (options.layout.height && options.layout.scroll) {
          const theadHeight = $(datatable.tableHead).find('.' + pfx + 'datatable-row').outerHeight()
          const tfootHeight = $(datatable.tableFoot).find('.' + pfx + 'datatable-row').outerHeight()
      ...
      setupTemplateCell (tablePart) {
        ...
        $(tablePart).find('.' + pfx + 'datatable-row').each(function (tri, tr) {
          ...
          // @deprecated in v5.0.6
          const rowCallback = Plugin.getOption('rows.callback')
          if (typeof rowCallback === 'function') {
            rowCallback($(tr), obj, tri)
          }
          // before template row callback
          const beforeTemplate = Plugin.getOption('rows.beforeTemplate')
          ...
          if (typeof obj === 'undefined') {
            ...
            $(tr).find('.' + pfx + 'datatable-cell').each(function (tdi, td) {
              ...
              const column = $.grep(columns, function (n, i) {
          ...
          const afterTemplate = Plugin.getOption('rows.afterTemplate')
      ...
      setupSystemColumn () {
        ...
        const initCheckbox = function (tr) {
          // get column settings by field
          const column = $.grep(columns, function (n, i) {
          ...
          if (typeof column !== 'undefined') {
            // enable column selector
            if (typeof column.selector !== 'undefined' && column.selector !== false) {
              const td = $(tr).find('[data-field="' + column.field + '"]')
              // check if checkbox exist
              if ($(td).find('.' + pfx + 'checkbox [type="checkbox"]').length > 0) {
                return
              }
      ...
      adjustCellsWidth () {
        ...
        const columns = $(datatable.tableBody)
              ...
              if (typeof dataWidth !== 'undefined') {
                if (dataWidth === 'auto') {
                  ...
                  if (Plugin.maxWidthList[field]) {
                    ...
                    const cells = $(datatable.table).find('.' + pfx + 'datatable-cell[data-field="' + field + '"]')
      ...
      adjustCellsHeight () {
        $.each($(datatable.table).children(), function (part, tablePart) {
          const totalRows = $(tablePart).find('.' + pfx + 'datatable-row').first().parent().find('.' + pfx + 'datatable-row').length
          for (let i = 1; i <= totalRows; i++) {
            const rows = $(tablePart).find('.' + pfx + 'datatable-row:nth-child(' + i + ')')
            if ($(rows).length > 0) {
              const maxHeight = Math.max.apply(null, $(rows).map(function () {
      ...
      scrollbar () {
        const scroll = {
          ...
          initPosition: null,
          init () {
            const screen = util.getViewPort().width
            // setup scrollable datatable
            if (options.layout.scroll) {
              // add scrollable datatable class
              $(datatable.wrap).addClass(pfx + 'datatable-scroll')

              const scrollable = $(datatable.tableBody).find('.' + pfx + 'datatable-lock-scroll')
          ...
          initDefaultScrollbar (scrollable) {
          ...
          initCustomScrollbar (scrollable) {
      ...
      setHeadTitle (tablePart) {
        ...
        const ths = tablePart.getElementsByTagName('td')
        ...
        $.each(columns, function (i, column) {
          ...
          if (typeof column.textAlign !== 'undefined') {
            const align = typeof datatable.textAlign[column.textAlign] !== 'undefined' ? datatable.textAlign[column.textAlign] : ''
      ...
      dataRender (action) {
        ...
        const buildMeta = function () {
          ...
          const meta = Plugin.getDataSourceParam('pagination')
          if (meta.perpage === 0) {
            meta.perpage = options.data.pageSize || 10
          }
          meta.total = datatable.dataSet.length
          const start = Math.max(meta.perpage * (meta.page - 1), 0)
          const end = Math.min(start + meta.perpage, meta.total)
          datatable.dataSet = $(datatable.dataSet).slice(start, end)
          return meta
        }

        const afterGetData = function (result) {
          const localPagingCallback = function (ctx, meta) {
          ...
          if (options.pagination) {
            if (options.data.serverPaging && options.data.type !== 'local') {
              // server pagination
              const serverMeta = Plugin.getObject('meta', result || null)
              if (serverMeta !== null) {
      ...
      insertData () {
        datatable.dataSet = datatable.dataSet || []
        const params = Plugin.getDataSourceParam()

        // get row attributes
        const pagination = params.pagination
        const start = (Math.max(pagination.page, 1) - 1) * pagination.perpage
        const end = Math.min(pagination.page, pagination.pages) * pagination.perpage
        let rowProps = {}
        ...
        const colLength = options.columns.length
        ...
        $.each(datatable.dataSet, function (rowIndex, row) {
          ...
          for (let a = 0; a < colLength; a += 1) {
            const column = options.columns[a]
            const classes = []
      ...
      getData () {
        ...
        if (options.data.type === 'remote') {
          const data = Plugin.getDataSourceParam()
      ...
      paging (meta, callback) {
        const pg = {
          ...
          serverCallback (ctx, meta) {
            Plugin.dataRender()
          },
          populate () {
            const icons = Plugin.getOption('layout.icons.pagination')
            const title = Plugin.getOption('translate.toolbar.pagination.items.default')
            // pager root element
            pg.pager = $('<div/>').addClass(pfx + 'datatable-pager ' + pfx + 'datatable-paging-loaded')
            // numbering links
            const pagerNumber = $('<ul/>').addClass(pfx + 'datatable-pager-nav mb-5 mb-sm-0')
            ...
            const pagesNumber = Plugin.getOption('toolbar.items.pagination.pages.desktop.pagesNumber')
            let end = Math.ceil(pg.meta.page / pagesNumber) * pagesNumber
          ...
          addPaginateEvent (e) {
            ...
            $(datatable).off(pg.paginateEvent).on(pg.paginateEvent, function (e, meta) {
              ...
              pg.pager = $(datatable.table).siblings('.' + pfx + 'datatable-pager')
              const pagerNumber = $(pg.pager).find('.' + pfx + 'datatable-pager-nav')
          ...
          updateInfo () {
            const start = Math.max(pg.meta.perpage * (pg.meta.page - 1) + 1, 1)
            const end = Math.min(start + pg.meta.perpage - 1, pg.meta.total)
            // page info update
          ...
          pagingBreakpoint () {
            ...
            const pagerNumber = $(datatable.table).siblings('.' + pfx + 'datatable-pager').find('.' + pfx + 'datatable-pager-nav')
            if ($(pagerNumber).length === 0) {
              return
            }

            const currentPage = Plugin.getCurrentPage()
            const pagerInput = $(pagerNumber).find('.' + pfx + 'datatable-pager-input').closest('li')
              ...
              function (mode, option) {
                if (util.isInResponsiveRange(mode)) {
                  switch (mode) {
                    case 'desktop':
                    case 'tablet':
                      const end = Math.ceil(currentPage / option.pagesNumber) * option.pagesNumber
                      const start = end - option.pagesNumber
                      $(pagerInput).hide()
           ...
           */
          paginationUpdate () {
            const pager = $(datatable.table).siblings('.' + pfx + 'datatable-pager').find('.' + pfx + 'datatable-pager-nav')
            const pagerMorePrev = $(pager).find('.' + pfx + 'datatable-pager-link-more-prev')
            const pagerMoreNext = $(pager).find('.' + pfx + 'datatable-pager-link-more-next')
            const pagerFirst = $(pager).find('.' + pfx + 'datatable-pager-link-first')
            const pagerPrev = $(pager).find('.' + pfx + 'datatable-pager-link-prev')
            const pagerNext = $(pager).find('.' + pfx + 'datatable-pager-link-next')
            const pagerLast = $(pager).find('.' + pfx + 'datatable-pager-link-last')

            // get visible page
            const pagerNumber = $(pager).find('.' + pfx + 'datatable-pager-link-number')
            // get page before of first visible
            const morePrevPage = Math.max($(pagerNumber).first().data('page') - 1, 1)
            ...
            const moreNextPage = Math.min($(pagerNumber).last().data('page') + 1,
              pg.meta.pages)
            ...
            const nav = Plugin.getOption('toolbar.items.pagination.navigation')
            if (!nav.first) {
              $(pagerFirst).remove()
            }
            if (!nav.prev) {
      ...
      columnHide () {
        ...
        $.each(options.columns, function (i, column) {
          if (typeof column.responsive !== 'undefined' || typeof column.visible !== 'undefined') {
            ...
            const tds = $.grep($(datatable.table).find('.' + pfx + 'datatable-cell'), function (n, i) {
       ...
       */
      setupSubDatatable () {
        const subTableCallback = Plugin.getOption('detail.content')
        ...
        const toggleSubTable = function (e) {
          e.preventDefault()
          // get parent row of this subtable
          const parentRow = $(this).closest('.' + pfx + 'datatable-row')

          // get subtable row for sub table
          let subTableRow = $(parentRow).next('.' + pfx + 'datatable-row-subtable')
          ...
          const subTable = $(subTableRow).find('.' + pfx + 'datatable-subtable')

          // get id from first column of parent row
          const primaryKey = $(this).closest('[data-field]:first-child').find('.' + pfx + 'datatable-toggle-subtable').data('value')

          const icon = $(this).find('i').removeAttr('class')
       ...
       * @param target
       */
      spinnerCallback (block, target) {
        if (typeof target === 'undefined') {
          target = datatable
        }
        // get spinner options
        const spinnerOptions = Plugin.getOption('layout.spinner')
        // spinner is disabled
          ...
          switch (type) {
            case 'date':
              if (typeof moment === 'undefined') {
                throw new TypeError('Moment.js is required.')
              }
      ...
      autoHide () {
        ...
        // force hide enabled
        const hidDefault = $(datatable.table).find('[data-autohide-enabled]')
        if (hidDefault.length) {
          hiddenExist = true
          hidDefault.hide()
        }

        const toggleHiddenColumns = function (e) {
          e.preventDefault()

          const row = $(this).closest('.' + pfx + 'datatable-row')
          let detailRow = $(row).next()

          if (!$(detailRow).hasClass(pfx + 'datatable-row-detail')) {
            $(this).find('i').removeClass(Plugin.getOption('layout.icons.rowDetail.collapse')).addClass(Plugin.getOption('layout.icons.rowDetail.expand'))

            const hiddenCells = $(row).find('.' + pfx + 'datatable-cell:hidden')
            const clonedCells = hiddenCells.clone().show()

            detailRow = $('<tr/>').addClass(pfx + 'datatable-row-detail').insertAfter(row)
            const detailRowTd = $('<td/>').addClass(pfx + 'datatable-detail').attr('colspan', Plugin.getTotalColumns()).appendTo(detailRow)

            const detailSubTable = $('<table/>')
            $(clonedCells).each(function () {
              const field = $(this).data('field')
              const column = $.grep(options.columns, function (n, i) {
          ...
          $(datatable.tableBody).each(function () {
            ...
            while ($(this)[0].offsetWidth < $(this)[0].scrollWidth && recursive < options.columns.length) {
              $(datatable.table).find('.' + pfx + 'datatable-row').each(function (i) {
                const cell = $(this).find('.' + pfx + 'datatable-cell:not(:hidden):not([data-autohide-disabled])').last()
                $(cell).hide()
                hiddenExist = true
              })
              recursive++
      ...
      setAutoColumns () {
        if (Plugin.getOption('data.autoColumns')) {
          $.each(datatable.dataSet[0], function (k, v) {
            const found = $.grep(options.columns, function (n, i) {
              return k === n.field
            })
            if (found.length === 0) {
       ...
       */
      isLocked () {
        const isLocked = Plugin.lockEnabledColumns()
       ...
       * @returns {number}
       */
      getExtraSpace (element) {
        const padding = parseInt($(element).css('paddingRight')) +
          parseInt($(element).css('paddingLeft'))
        const margin = parseInt($(element).css('marginRight')) +
       ...
       * @returns {*}
       */
      getOneRow (tablePart, row, tdOnly) {
        if (typeof tdOnly === 'undefined') {
          tdOnly = true
        }
        // get list of <tr>
        let result = $(tablePart).find('.' + pfx + 'datatable-row:not(.' + pfx + 'datatable-row-detail):nth-child(' + row + ')')
        if (tdOnly) {
      ...
      sortColumn (header, sort, int) {
        ...
        let rows = $(datatable.tableBody).find('.' + pfx + 'datatable-row')
        const hIndex = $(header).closest('.' + pfx + 'datatable-lock').index()
        if (hIndex !== -1) {
          rows = $(datatable.tableBody).find('.' + pfx + 'datatable-lock:nth-child(' + (hIndex + 1) + ')').find('.' + pfx + 'datatable-row')
        }

        const container = $(rows).parent()
      ...
      sorting () {
        const sortObj = {
          ...
          setIcon () {
            const meta = Plugin.getDataSourceParam('sort')
            if ($.isEmptyObject(meta)) {
              return
            }

            const column = Plugin.getColumnByField(meta.field)
            ...
            const td = $(datatable.tableHead).find('.' + pfx + 'datatable-cell[data-field="' + meta.field + '"]').attr('data-sort', meta.sort)
            const sorting = $(td).find('span')
            const icon = $(sorting).find('i')

            const icons = Plugin.getOption('layout.icons.sort')
          ...
          sortClick (e) {
            ...
            const field = $(this).data('field')
            const column = Plugin.getColumnByField(field)
            // sort is disabled for this column
            if (typeof column === 'undefined') {
              return
            }
      ...
      localDataUpdate () {
        ...
        const field = Plugin.getObject('sort.field', params)
        const sort = Plugin.getObject('sort.sort', params)
        const column = Plugin.getColumnByField(field)
        ...
        if (typeof params.query === 'object' && !Plugin.getOption('data.serverFiltering')) {
          ...
          const nestedSearch = function (obj) {
            for (const field in obj) {
              if (!obj.hasOwnProperty(field)) {
                continue
              }
              if (typeof obj[field] === 'string') {
                if (obj[field].toLowerCase() == search || obj[field].toLowerCase().includes(search)) {
                  return true
                }
       ...
       * @returns {*}
       */
      filterArray (list, args, operator) {
        ...
        $.each(list, function (key, obj) {
          ...
          $.each(args, function (m_key, m_value) {
            m_value = Array.isArray(m_value) ? m_value : [m_value]
            const match_property = Plugin.getObject(m_key, to_match)
            if (typeof match_property !== 'undefined' && match_property) {
              const lhs = match_property.toString().toLowerCase()
              m_value.forEach(function (item, index) {
                if (item.toString().toLowerCase() == lhs || lhs.includes(item.toString().toLowerCase())) {
                  matched++
                }
              })
            }
      ...
      getHiddenDimensions (element, includeMargin) {
        ...
        const hiddenParents = $(element).parents().addBack().not(':visible')
      ...
      getGeneralSearchKey () {
        const searchInput = $(Plugin.getOption('search.input'))
      ...
      getRecord (id) {
        ...
        $(datatable.tableBody).find('.' + pfx + 'datatable-cell:first-child').each(function (i, cell) {
          if (id == $(cell).text()) {
            const rowNumber = $(cell).closest('.' + pfx + 'datatable-row').index() + 1
      ...
      setActive (cell) {
        ...
        $(cell).each(function (i, td) {
          // normal table
          const row = $(td).closest('tr').addClass(pfx + 'datatable-row-active')

          const id = $(td).attr('value')
          if (typeof id !== 'undefined') {
            ids.push(id)
          }
        })
      ...
      setInactive (cell) {
        ...
        $(cell).each(function (i, td) {
          // normal table
          const row = $(td).closest('tr').removeClass(pfx + 'datatable-row-active')
       ...
       */
      setActiveAll (active) {
        const checkboxes = $(datatable.table)
          .find('> tbody, > thead')
      ...
      search (value, columns) {
        ...
        delay(function () {
          ...
          const query = Plugin.getDataSourceQuery()

          // search not by columns
          if (typeof columns === 'undefined' && typeof value !== 'undefined') {
            const key = Plugin.getGeneralSearchKey()
            query[key] = value
          }
       ...
       * @param value
       */
      setDataSourceParam (param, value) {
      ...
      columns (selector) {
        ...
        const columns = $(context).find('.' + pfx + 'datatable-cell[data-field="' + selector + '"]')
        if (columns.length > 0) {
      ...
      cells (selector) {
        let cells = $(datatable.tableBody).find('.' + pfx + 'datatable-cell')
      ...
      visible (bool) {
        if ($(Plugin.recentNode.length)) {
          const locked = Plugin.lockEnabledColumns()
          if (Plugin.recentNode === Plugin.nodeCols) {
            var index = Plugin.recentNode.index()

            if (Plugin.isLocked()) {
              const scrollColumns = $(Plugin.recentNode).closest('.' + pfx + 'datatable-lock-scroll').length
              if (scrollColumns) {
    ...
    if (typeof options !== 'undefined') {
      ...
      } else if (!datatable.data(pluginName) && !$(this).hasClass(pfx + 'datatable-loaded')) {
  ...
  $.fn[pluginName].checkbox = function (datatable, options) {
    const Extension = {
      ...
      initVars () {
        ...
        const storage = datatable.stateGet('checkbox')
